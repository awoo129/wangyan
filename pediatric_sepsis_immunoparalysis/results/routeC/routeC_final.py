...
def main():
    ...
    print(f"\n样本标题数: {len(sample_titles_66099)}")
    print(f"表达矩阵列数: {len(expression_data_66099[0]) if expression_data_66099 else 0}")
    print("\n样本标题示例:")
    for i, title in enumerate(sample_titles_66099[:5]):
        print(f"  {title}")
    
    print("\n注意: GSE66099是6个儿童脓毒症研究的合并数据集")
    print("由于series_matrix文件缺少分组信息，无法直接区分疾病严重程度")
    ...
    for gene in ['CIITA', 'HLA-DRA', 'CD74']:
        if gene in gene_expr_66099:
            results_66099[gene] = gene_expr_66099[gene]
    ...
    print("│                           数据集评估结果                                     │")
    print("├─────────────────────────────────────────────────────────────────────────────┤")
    print("│ 1. GSE13904 数据集:                                                         │")
    print("│    ✓ 样本组成明确: 儿童对照、SIRS、脓毒症、脓毒性休克 Day1样本               │")
    print("│    ✓ 可用于分析CIITA等MHC II基因在不同疾病严重程度下的表达                     │")
    print("│    结论: 可作为外部验证数据集                                                │")
    print("│                                                                             │")
    print("│ 2. GSE66099 数据集:                                                         │")
    print("│    ✓ 包含276个儿童样本                                                      │")
    print("│    ✗ series_matrix文件缺少分组标注                                          │")
    print("│    结论: 不适合直接用于验证（需要获取原始元数据）                             │")
    print("│                                                                             │")
    print("│ 3. 年龄异质性验证:                                                          │")
    print("│    路线A: 儿童脓毒症数据 → CIITA表达模式已知                                │")
    print("│    路线B: 成人脓毒症数据 → CIITA表达模式已知                                │")
    print("│    当前: 两个外部验证数据集都是儿童数据                                     │")
    print("│    缺口: 需要成人外部数据集进行年龄分层验证                                  │")
    print("└─────────────────────────────────────────────────────────────────────────────┘")
