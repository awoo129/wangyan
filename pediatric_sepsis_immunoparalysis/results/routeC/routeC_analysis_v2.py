...
def parse_series_matrix(filepath, is_gz=False):
    ...
    with opener(filepath, 'rt') as f:
        for line in f:
            ...
            # 解析样本元信息
            if line.startswith('!') and not reading_data:
                if '\t' in line:
                    parts = line.split('\t')
            ...
            # 读取表达数据行
            if reading_data and line and not line.startswith('!') and '\t' in line:
                parts = line.split('\t')
                if parts:
...
def main():
    ...
    # GSE66099是合并数据集，来自多个研究
    # 根据PMID 25972003，该数据集包含276个Day1样本
    # 来自GSE4607, GSE8121, GSE9692, GSE13904, GSE26378, GSE26440
    print("\n注意: GSE66099是6个儿童脓毒症研究的合并数据集")
    print("由于series_matrix文件不包含分组信息，需要通过其他方式确定样本分组")
    ...
    for gene in gene_expr_66099:
        ...
        print(f"  range=[{results_66099[gene]['min']:.4f}, {results_66099[gene]['max']:.4f}]")
    ...
    if 'CIITA' in gene_expr_13904_day1 and 'CIITA' in gene_expr_66099:
        ...
        print(f"\nCIITA表达:")
        print(f"  GSE13904: mean={np.mean(ciita_13904):.4f} ± {np.std(ciita_13904):.4f}")
        print(f"  GSE66099: mean={np.mean(ciita_66099):.4f} ± {np.std(ciita_66099):.4f}")
        
        # 分布对比
        print(f"\n  GSE13904 CIITA分布:")
        print(f"    Control (n={sum(1 for g in sample_groups_day1 if g=='Control')}): {np.mean([ciita_13904[i] for i,g in enumerate(sample_groups_day1) if g=='Control']):.4f}")
        print(f"    Sepsis (n={sum(1 for g in sample_groups_day1 if g=='Sepsis')}): {np.mean([ciita_13904[i] for i,g in enumerate(sample_groups_day1) if g=='Sepsis']):.4f}")
        print(f"    SepticShock (n={sum(1 for g in sample_groups_day1 if g=='SepticShock')}): {np.mean([ciita_13904[i] for i,g in enumerate(sample_groups_day1) if g=='SepticShock']):.4f}")
